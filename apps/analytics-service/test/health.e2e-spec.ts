...
describe('Health endpoints (e2e)', () => {
  ...
  afterAll(()=>app.close());
  it('GET /health/live',()=>request(app.getHttpServer()).get('/health/live').expect(200).expect(({body})=>expect(body).toMatchObject({status:'ok',service:'analytics-service'})));
  it('GET /health/ready',()=>request(app.getHttpServer()).get('/health/ready').expect(200).expect(({body})=>expect(body).toMatchObject({status:'ok',service:'analytics-service'})));
